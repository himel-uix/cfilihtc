...
const Support = () => {
  ...
  return (
    <section className='py-6 bg-[#0F1523] border-y border-[#0EA5E952]'>
        <div className="container mx-auto px-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-11">
            {supportItems.map((item)=>(
                    <div className='flex items-center gap-3 md:pr-4 lg:pr-11 sm:border-r border-white' key={item.id}>
                        <Image src={item.icon} alt={item.title} width={64} height={64} className='size-16' />
                        <h3 className='font-medium text-lg md:text-xl lg:text-2xl text-white'>{item.title}</h3>
                    </div>
                ))}
        </div>
    </section>
